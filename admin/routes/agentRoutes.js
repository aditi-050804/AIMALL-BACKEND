...
route.delete('/:id', verifyToken, async (req, res) => {
  try {
    ...
    // 1. If requester is Admin -> Proceed with Hard Delete (Moderation Action)
    // We allow admins to hard-delete even if they are the owner, to ensure administrative control
    if (isAdmin) {
      console.log(`[DELETE AGENT] User ${req.user.id} is ADMIN. Performing Hard Delete on Agent ${agent._id}.`);
    ...
    // 2. If NOT Admin, but IS Owner -> Initiating Pending Deletion
    if (agent.owner && agent.owner.toString() === req.user.id.toString()) {
...
// Approve Agent Deletion (Admin Action)
route.post('/admin/approve-deletion/:id', verifyToken, async (req, res) => {
  try {
    const requestor = await userModel.findById(req.user.id);
    ...
    try {
      ...
      const notifications = uniqueBuyers.map(userId => ({
        userId,
        message: `Status Update: '${agent.agentName}' has been removed from the platform by the vendor. Active subscriptions have been terminated.`,
        type: 'warning',
        role: 'user',
        targetId: agent._id
      }));
    ...
    res.json({ message: "Deletion request approved. Agent permanently removed.", agentId: req.params.id });
  } catch (err) {
...
// Reject Agent Deletion (Admin Action)
route.post('/admin/reject-deletion/:id', verifyToken, async (req, res) => {
  try {
    const requestor = await userModel.findById(req.user.id);
    if (requestor?.role !== 'admin') {
    ...
    agent.deletionStatus = 'None';
    await agent.save();

    // Notify Vendor
    const notification = new notificationModel({
      userId: agent.owner,
      message: `Your deletion request for '${agent.agentName}' has been rejected by moderation. Reason: ${reason || 'Incomplete reason provided.'}`,
      type: 'info',
      role: 'vendor',
      targetId: agent._id
    });
    ...
    res.json({ message: "Deletion request rejected. Agent restored to active inventory.", agentId: req.params.id });
  } catch (err) {
